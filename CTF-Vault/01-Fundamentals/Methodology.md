...
ดู [[Passive-Recon]], [[Active-Recon]], [[Enumeration]]
...
- เห็น `?file=home.html` → ลอง [[LFI-RFI|LFI]]
- เห็น `?url=...` → ลอง [[SSRF]]
- เห็น `?id=1` → ลอง [[SQL-Injection|SQLi]]
- เห็น textbox ที่จะถูก render ในหน้า → ลอง [[XSS]]
- เห็น search ที่ render ค่าใน template → ลอง [[SSTI]]
- เห็น JWT token → ลอง [[JWT-Attacks]]
...
- ลุย [[Passive-Recon|Recon ก่อนเลย]]
- หรือเลือก [[03-Web-Exploitation/SQL-Injection|Web]]
- กลับ [[00-START-HERE]]
